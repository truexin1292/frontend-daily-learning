...
class Board extends React.Component {
    getBorderColor(displayValue) {
        ...
        else if (displayValue === '') //No mines nearby
            borderColor = '#f4f4f4';
    ...
    renderSquare(columnIndex, rowIndex) {
        ...
        return React.createElement(Square, {
            ...
            fillColor: fillColor,
            onClick: e => this.props.onClick(e, columnIndex, rowIndex),
            key: key
        });
    }
...
class Game extends React.Component {
    ...
    generateRandomMines() {
        ...
        while (minesOnBoard < this.minesNumber) {
            var index = Math.floor(Math.random() * totalSquaresNumber);
            // returns a number between 0 and number of squares
            if (this.squaresValues[Math.floor(index / this.columns)][index % this.columns] === 0) {
                //square does not have a mine already
                this.squaresValues[Math.floor(index / this.columns)][index % this.columns] = 10;
                //10 value will represent a mine
                minesOnBoard++;
            }
        }
    }
    ...
    handleClick(e, columnIndex, rowIndex) {
        ...
        if (e.shiftKey && (this.state.displayedSquares[rowIndex][columnIndex] === null ||
    ...
    revealAdjacentMinesNumber(rowIndex, columnIndex) {
        ...
        if (this.state.displayedSquares[rowIndex][columnIndex] === ''
            || this.state.displayedSquares[rowIndex][columnIndex] === 'f') {
            return;
        }
...
class GameSettings extends React.Component {
    ...
    handleTextChange(e) {
        ...
        if (e.target.id === 'rowsTxt') {
            ...
            //     rows: value
            // });
            this.props.startNewGame(value, columns, mines);
        } else if (e.target.id === 'columnsTxt') {
            // this.setState({
            //     columns: value
            // });
            this.props.startNewGame(rows, value, mines);
        } else if (e.target.id === 'minesTxt') {
            // this.setState({
            //     mines: value
            // });
            this.props.startNewGame(rows, columns, value);
        }
    }

    OnClickRestart() {
        if (!isNaN(parseInt(this.state.rows))
            && !isNaN(parseInt(this.state.columns))
            && !isNaN(parseInt(this.state.mines))) {
            ...
            React.createElement(
                ...
                React.createElement(
                    ...
                    React.createElement(
                        'img',
                        {
                            src: './assets/arrowsLeftRight.svg',
                            width: '20',
                            height: '20'
                        }),
                    React.createElement(
    ...
    render() {
        ...
        return React.createElement(
            ...
            React.createElement(
                ...
                '&copy; MineSweeper by Naama Avni'
            )
        );
    }
}
...
ReactDOM.render(
    ...
    document.getElementById('root') // ID选择器才能直接选中元素，不需要像标签和class选择器的【0】
);
var t1 = performance.now(); // 返回时间戳毫秒值，更加精准
// Date.now(); 依赖操作系统时间设置； Date.parse(2015/02/09 09:00:00)/Date.parse(2015-02-09 09:00:00); 都是返回时间戳
